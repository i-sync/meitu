...
class HandsomeSpider(scrapy.Spider):
    ...
    start_urls = ['http://www.meijuntu.com/handsome']
    ...
    def parse(self, response):
        ...
        for l in links:
            ...
            item["contents"] = []
            yield scrapy.Request(url = f"{self.base_url}{link}", callback=self.detail_parse, meta={"item": item, "request_type": "album", "category_name": "handsome", "album_name": name })
            # break
        ...
        if page_number <= 18:
            next_url = f"{self.base_url}/handsome/index-{page_number}.html"
    ...
    def detail_parse(self, response):
        ...
        for index in range(2, int(total)+1):
            pages.append(f'/handsome/{item["name"]}-{index}.html')
        if len(pages) > 0:
            link = pages.pop(0)
