...
from app.library.models import (MeituCategory, MeituContent, MeituImage,
                                MeituMedia, MeituMediaModel, MeituMediaTag,
...
async def news(request: Request, page = "1", order = "new"):
    page_index = get_page_index(page)

    with session_scope() as session:
        rows = session.query(func.count(MeituMedia.id)).filter(MeituMedia.category_name == 'news', MeituMedia.is_enabled == 1).scalar()
        ...
        if request.state.user_agent.is_mobile:
            tops = session.query(MeituMedia).filter(MeituMedia.category_name == 'news', MeituMedia.is_enabled == 1).order_by(desc(MeituMedia.view_count)).limit(10).all()
        else:
            tops = []
...
@router.get('/news/{name}', response_class=HTMLResponse)
async def news_detail(request: Request, name, page="1"):
    if not name:
        return RedirectResponse("/news")

    page_index = get_page_index(page)
    with session_scope() as session:
        media = session.query(MeituMedia).filter(MeituMedia.category_name == 'news', MeituMedia.name == name, MeituMedia.is_enabled == 1).first()
        ...
        if len(media.tags):
            ...
            sql = text("""SELECT media.id, media.name, media.title, media.cover, media.origin_created_at, media.view_count,  COUNT(1) AS tag_count
                ...
                WHERE media.category_name = 'news' AND T1.media_id = :media_id
                GROUP BY T2.media_id
                ORDER BY tag_count DESC
                limit 20;""")
            media.related = session.execute(sql, {'media_id': media.id}).fetchall()
        else:
            media.related = session.query(MeituMedia).filter(MeituMedia.category_name == 'news', MeituMedia.is_enabled == 1).limit(20).all()


        if not request.state.user_agent.is_mobile:
            tops = session.query(MeituMedia).filter(MeituMedia.category_name == 'news', MeituMedia.id != media.id, MeituMedia.is_enabled == 1).order_by(desc(MeituMedia.view_count)).limit(10).all()
            latest = session.query(MeituMedia).filter(MeituMedia.category_name == 'news', MeituMedia.id != media.id, MeituMedia.is_enabled == 1).order_by(desc(MeituMedia.origin_created_at)).limit(10).all()
        else:
            tops = []
            latest = []
    ...
    return templates.TemplateResponse("news-detail.html", {"request": request, "data": data})
